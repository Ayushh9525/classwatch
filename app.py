...
app.config['SESSION_COOKIE_SECURE'] = env_flag('CLASSWATCH_SECURE_COOKIE', False)


def build_ice_servers():
    """
    Build WebRTC ICE server config from environment variables.

    Defaults to public Google STUN locally. In production, TURN credentials can
    be injected through env vars without changing the frontend bundle.
    """
    stun_urls = os.environ.get('CLASSWATCH_STUN_URLS', 'stun:stun.l.google.com:19302')
    servers = []

    for url in [u.strip() for u in stun_urls.split(',') if u.strip()]:
        servers.append({'urls': url})

    turn_urls = os.environ.get('CLASSWATCH_TURN_URLS', '').strip()
    turn_username = os.environ.get('CLASSWATCH_TURN_USERNAME', '').strip()
    turn_credential = os.environ.get('CLASSWATCH_TURN_CREDENTIAL', '').strip()
...
if __name__ == '__main__':
    ...
    debug = env_flag('FLASK_DEBUG', False) or env_flag('CLASSWATCH_DEBUG', False)
