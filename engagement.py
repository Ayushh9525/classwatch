...
class EngagementDetector:
    ...
    def __init__(self):
        ...
        self.writing_event = SustainedState(self.writing_hold_sec, clear_sec=1.0)
    ...
    def _load_phone_model(self):
        ...
        if not os.path.exists(self.phone_model_path):
            self._maybe_download_phone_model()
        if not os.path.exists(self.phone_model_path):
            return None
        try:
    ...
    def _maybe_download_phone_model(self):
        """Optionally download YOLO weights on deploy if they are missing."""
        if os.environ.get("CLASSWATCH_AUTO_DOWNLOAD_YOLO", "1").lower() in {"0", "false", "no", "off"}:
            return
        ...
        target_dir = os.path.dirname(self.phone_model_path)
        if not target_dir:
            return

        tmp_path = f"{self.phone_model_path}.tmp"
        try:
            os.makedirs(target_dir, exist_ok=True)
            urllib.request.urlretrieve(model_url, tmp_path)
            os.replace(tmp_path, self.phone_model_path)
        except Exception:
            try:
    ...
    def analyze_frame(self, frame_data):
        ...
        try:
            ...
            looking_down_now = pitch > 14.0 and nose_y > 0.56
            centered_now = abs(yaw) < 14.0 and 0.28 <= nose_x <= 0.72
            eyes_available = avg_ear > (self.ear_threshold * 0.92)
            writing_now = looking_down_now and centered_now and eyes_available and not phone_visible_now

            if self.writing_event.update(writing_now):
                self.away_event.update(False)
                result["writing"] = True
